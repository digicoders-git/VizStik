    ...
    <div className="p-6">
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            <MdPerson size={16} style={{ color: colors.primary }} />
            DS Name: {employee.dsName}
          </div>
          <div
            className="flex items-center gap-2 text-sm"
            style={{ color: colors.text }}
                  ...
                  .map((shop, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm">
                        <button
                          onClick={() =>
                            navigate(`/dashboard/shops/view/${shop._id}`)
                          }
      ...
      {employee.addedShops && employee.addedShops.length > itemsPerPage && (
        ...
        >
          <div className="text-sm">
            ...
            {Math.min(currentPage * itemsPerPage, employee.addedShops.length)}{" "}
            of {employee.addedShops.length} entries
          </div>
          <div className="flex items-center gap-2">
            <button
            ...
            <div className="flex gap-1">
              {Array.from(
                {
                  length: Math.ceil(employee.addedShops.length / itemsPerPage),
                },
                (_, i) => i + 1,
              )
                .filter((page) => {
                  const totalPages = Math.ceil(
                    employee.addedShops.length / itemsPerPage,
                  );
                  return (
                    page === 1 ||
                    page === totalPages ||
            ...
            <button
              onClick={() =>
                setCurrentPage((prev) =>
                  Math.min(
                    prev + 1,
                    Math.ceil(employee.addedShops.length / itemsPerPage),
                  ),
                )
              }
              disabled={
